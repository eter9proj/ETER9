...
<div class="intro-wrapper media" id="intro-{{$intro_id}}">
	...
	<div class="media-body">
		...
		<form class="intro-form" action="notification/{{$intro_id}}" method="post">
			<button class="btn btn-small btn-primary" type="button" onclick="addElmToModal('#intro-approve-wrapper-{{$intro_id}}');"><i class="fa ri-check-line" aria-hidden="true"></i> {{$approve}}</button>
			{{if $discard}}
				<button class="btn btn-small btn-warning intro-submit-discard" type="submit" name="submit" value="{{$discard}}"><i class="fa ri-delete-bin-line" aria-hidden="true"></i> {{$discard}}</button>
			{{/if}}
			<button class="btn btn-small btn-danger intro-submit-ignore" type="submit" name="submit" value="{{$ignore}}"><i class="fa ri-user-forbid-line" aria-hidden="true"></i> {{$ignore}}</button>
		</form>
		{{else}}
		{{* The intro actions like approve, ignore, discard intro*}}
		<div class="intro-actions pull-right nav-pills preferences">
			<button class="btn-link intro-action-link" onclick="addElmToModal('#intro-approve-wrapper-{{$intro_id}}');" aria-label="{{$approve}}" title="{{$approve}}" data-toggle="tooltip"><i class="fa ri-check-line" aria-hidden="true"></i></button>

			<form class="intro-form" action="notification/{{$intro_id}}" method="post">
				<button class="btn-link intro-submit-ignore intro-action-link" type="submit" name="submit" value="{{$ignore}}" aria-label="{{$ignore}}" title="{{$ignore}}" data-toggle="tooltip"><i class="fa ri-user-forbid-line" aria-hidden="true"></i></button>
				{{if $discard}}<button class="btn-link intro-submit-discard intro-action-link" type="submit" name="submit" value="{{$discard}}" aria-label="{{$discard}}" title="{{$discard}}" data-toggle="tooltip"><i class="fa ri-delete-bin-line" aria-hidden="true"></i></button>{{/if}}
			</form>
		</div>
		{{/if}}

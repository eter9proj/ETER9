	...
	{{if $item.thread_level<3}}
		<button type="button" class="hide-comments-outer fakelink" onclick="showHideComments({{$item.id}});">
			<span id="hide-comments-total-{{$item.id}}" class="hide-comments-total">
				<i class="fa ri-arrow-right-s-fill" aria-hidden="true"></i>
				{{$item.num_comments}} - {{$item.show_text}}
			</span>
			<span id="hide-comments-{{$item.id}}" class="hide-comments" style="display: none">
				<i class="fa ri-arrow-down-s-fill" aria-hidden="true"></i>
...
{{/if}}
	...
	{{if $item.parentguid}}
		<span class="visible-sm-inline visible-xs wall-item-responses time">
			<i class="fa ri-reply-line" aria-hidden="true"></i> <a id="btn-{{$item.id}}" class="" href="javascript:;" onclick="scrollToItem('item-' + '{{$item.parentguid}}');">{{$item.inreplyto}}</a>
	...
	{{/if}}
	{{if $item.reshared}}
		<span class="wall-item-announce wall-item-responses time" id="wall-item-announce-{{$item.id}}"><i class="fa ri-repeat-line" aria-hidden="true"></i> {{$item.reshared nofilter}}</span>
	{{/if}}
		<p>
	...
	{{/if}}
				...
				<div class="additional-info text-muted">
					<div id="wall-item-ago-{{$item.id}}" class="wall-item-ago">
						<small>
							...
							{{/if}}
							{{if $item.pinned}}
								&bull; <i class="fa ri-pushpin-2-line" aria-hidden="true" title="{{$item.pinned}}"></i>
								<span class="sr-only">{{$item.pinned}}</span>
							{{/if}}
							{{if $item.connector}}
								&bull;
								<small><i class="fa ri-plug-line" title="{{$item.connector}}" aria-hidden="true"></i></small>
							{{else}}
								&bull;
								<span class="navicon lock fakelink" onClick="lockview(event, 'item', {{$item.id}});" title="{{$item.privacy}}" data-toggle="tooltip">
									<small><i class="fa {{if $item.private == 1}}ri-lock-line{{elseif $item.private == 0}}ri-global-line{{else}}fa-low-vision{{/if}}" aria-hidden="true"></i></small>
								</span>
							{{/if}}
						</small>
					</div>
					...
					<p class="text-muted">
						<small>
							...
							{{else}}
								&bull;
								<span class="navicon lock fakelink" onClick="lockview(event, 'item', {{$item.id}});" title="{{$item.privacy}}" data-toggle="tooltip">
								<i class="fa {{if $item.private == 1}}fa-lock{{elseif $item.private == 0}}ri-global-line{{else}}fa-low-vision{{/if}}" aria-hidden="true"></i>
								</span>
							{{/if}}
						</small>
					</p>
				...
				<small>
					{{if $item.parentguid}}
						<span class="hidden-xs hidden-sm">
							<a id="btn-{{$item.id}}" class="time" href="javascript:;" onclick="scrollToItem('item-' + '{{$item.parentguid}}');"><i class="fa ri-reply-line" aria-hidden="true"></i> {{$item.inreplyto}}</a>
							<i class="hidden-xs">&#x2022;</i>
						</span>
					{{else}}
						{{if $item.isunknown}}
					...
					{{else}}
						<span class="navicon lock fakelink" onClick="lockview(event, 'item', {{$item.id}});" title="{{$item.privacy}}" data-toggle="tooltip">
							&bull;
							<small><i class="fa {{if $item.private == 1}}fa-lock{{elseif $item.private == 0}}ri-global-line{{else}}fa-low-vision{{/if}}" aria-hidden="true"></i></small>
						</span>
					{{/if}}
				</small>
			</span>
		...
		{{if !$item.suppress_tags}}
			{{foreach $item.hashtags as $tag}}
				<span class="tag label btn-info sm">{{$tag nofilter}} <i class="fa ri-flashlight-line" aria-hidden="true"></i></span>
			{{/foreach}}

			{{foreach $item.mentions as $tag}}
				<span class="mention label btn-warning sm">{{$tag nofilter}} <i class="fa ri-user-3-fill" aria-hidden="true"></i></span>
			{{/foreach}}

			{{*foreach $item.implicit_mentions as $tag}}
				<span class="mention label label-default sm">{{$tag nofilter}} <i class="fa ri-eye-off-fill" aria-hidden="true"></i></span>
			{{/foreach*}}
		{{/if}}
		...
		<div class="wall-item-actions">
			...
			{{if $item.vote}}
				...
				{{if $item.vote.announce}}
					<button type="button" class="btn-link button-announces{{if $item.responses.announce.self}} active" aria-pressed="true{{/if}}" id="announce-{{$item.id}}" title="{{$item.vote.announce.0}}" onclick="doActivityItemAction({{$item.id}}, 'announce'{{if $item.responses.announce.self}}, true{{/if}});" ><i class="fa ri-repeat-line" aria-hidden="true"></i></button>
					<span title="{{$item.responses.announce.title}}">{{$item.responses.announce.total}}</span>
				{{else}}
					<button type="button" class="btn-link button-announces" id="announce-{{$item.id}}" title="{{$item.vote.announce.0}}" disabled><i class="fa ri-repeat-line" aria-hidden="true"></i></button>
				{{/if}}
				</span>
				<span class="wall-item-response">
				{{if $item.vote.share}}
					<button type="button" class="btn-link button-votes" id="share-{{$item.id}}" title="{{$item.vote.share.0}}" onclick="jotShare({{$item.id}});"><i class="fa ri-chat-quote-lin" aria-hidden="true"></i></button>
					<span title="{{$item.quoteshares.title}}">{{$item.quoteshares.total}}</span>
				{{else}}
					<button type="button" class="btn-link button-votes" id="share-{{$item.id}}" title="{{$item.vote.share.0}}" disabled><i class="fa ri-chat-quote-line" aria-hidden="true"></i></button>
				{{/if}}
				</span>
			{{/if}}
			...
			{{if $item.vote}}
				<span class="wall-item-response">
				{{if $item.vote.like}}
					<button type="button" class="btn-link button-likes{{if $item.responses.like.self}} active" aria-pressed="true{{/if}}" id="like-{{$item.id}}" title="{{$item.vote.like.0}}" onclick="doActivityItemAction({{$item.id}}, 'like'{{if $item.responses.like.self}}, true{{/if}});" ><i class="fa ri-thumb-up-line" aria-hidden="true"></i></button>
				...
				{{if $item.vote.dislike}}
					<button type="button" class="btn-link button-likes{{if $item.responses.dislike.self}} active" aria-pressed="true{{/if}}" id="dislike-{{$item.id}}" title="{{$item.vote.dislike.0}}" onclick="doActivityItemAction({{$item.id}}, 'dislike'{{if $item.responses.dislike.self}}, true{{/if}});" ><i class="fa ri-thumb-down-line" aria-hidden="true"></i></button>
					<span title="{{$item.responses.dislike.title}}">{{$item.responses.dislike.total}}</span>
				{{elseif !$item.hide_dislike}}
					<button type="button" class="btn-link button-likes" id="dislike-{{$item.id}}" title="{{$item.vote.dislike.0}}" disabled><i class="fa ri-thumb-down-line" aria-hidden="true"></i></button>
				{{/if}}
				</span>
		...
		<span class="more-links btn-group{{if $item.thread_level > 1}} dropup{{/if}}">
			<button type="button" class="btn-link dropdown-toggle" data-toggle="dropdown" id="dropdownMenuOptions-{{$item.id}}" aria-haspopup="true" aria-expanded="false" title="{{$item.menu}}"><i class="fa ri-more-2-line" aria-hidden="true"></i></button>
			<ul class="dropdown-menu dropdown-menu-right" role="menu" aria-labelledby="dropdownMenuOptions-{{$item.id}}">
				{{if $item.edpost}} {{* edit the posting *}}
				<li role="menuitem">
						<a href="javascript:editpost('{{$item.edpost.0}}?mode=none');" title="{{$item.edpost.1}}" class="btn-link navicon pencil"><i class="fa ri-edit-box-fill" aria-hidden="true"></i>&ensp;{{$item.edpost.1}}</a>
				</li>
				{{/if}}

				{{if $item.tagger}} {{* tag the post *}}
				<li role="menuitem">
						<a id="tagger-{{$item.id}}" href="javascript:itemTag({{$item.id}});" class="btn-link {{$item.tagger.class}}" title="{{$item.tagger.add}}"><i class="fa ri-price-tag-3-fill" aria-hidden="true"></i>&ensp;{{$item.tagger.add}}</a>
				</li>
				{{/if}}

				{{if $item.filer}}
				<li role="menuitem">
						<a id="filer-{{$item.id}}" href="javascript:itemFiler({{$item.id}});" class="btn-link filer-item filer-icon" title="{{$item.filer}}"><i class="fa ri-folder-fill" aria-hidden="true"></i>&ensp;{{$item.filer}}</a>
				</li>
				{{/if}}

				{{if $item.pin}}
				<li role="menuitem">
						<a id="pin-{{$item.id}}" href="javascript:doPin({{$item.id}});" class="btn-link {{$item.pin.classdo}}" title="{{$item.pin.do}}"><i class="fa ri-pushpin-fill" aria-hidden="true"></i>&ensp;{{$item.pin.do}}</a>
						<a id="unpin-{{$item.id}}" href="javascript:doPin({{$item.id}});" class="btn-link {{$item.pin.classundo}}" title="{{$item.pin.undo}}"><i class="fa ri-unpin-fill" aria-hidden="true"></i>&ensp;{{$item.pin.undo}}</a>
				</li>
				{{/if}}

				{{if $item.star}}
				<li role="menuitem">
						<a id="star-{{$item.id}}" href="javascript:doStar({{$item.id}});" class="btn-link {{$item.star.classdo}}" title="{{$item.star.do}}"><i class="fa ri-star-fill" aria-hidden="true"></i>&ensp;{{$item.star.do}}</a>
				...
				<li role="menuitem">
						<a id="follow_thread-{{$item.id}}" href="javascript:{{$item.follow_thread.action}}" class="btn-link" title="{{$item.follow_thread.title}}"><i class="fa ri-add-box-line" aria-hidden="true"></i>&ensp;{{$item.follow_thread.title}}</a>
				</li>
				{{/if}}

				{{if $item.language}}
				<li role="menuitem">
						<a id="language-{{$item.id}}" href="javascript:displayLanguage({{$item.uriid}});" class="btn-link filer-item language-icon" title="{{$item.language}}"><i class="fa ri-translate-2" aria-hidden="true"></i>&ensp;{{$item.language}}</a>
				</li>
				{{/if}}

				<li role="menuitem">
						<a id="searchtext-{{$item.id}}" href="javascript:displaySearchText({{$item.uriid}});" class="btn-link filer-item search-icon" title="{{$item.searchtext}}"><i class="fa ri-search-2-fill" aria-hidden="true"></i>&ensp;{{$item.searchtext}}</a>
				</li>

				{{if $item.browsershare}}
				<li role="menuitem" class="button-browser-share">
						<a id="browser-share-{{$item.id}}" href="javascript:navigator.share({url: '{{$item.plink.orig}}'})" class="btn-link button-browser-share" title="{{$item.browsershare.1}}"><i class="fa ri-stackshare-line" aria-hidden="true"></i>&ensp;{{$item.browsershare.0}}</a>
				</li>
				{{/if}}
				...
				{{if $item.ignore}}
				<li role="menuitem">
						<a id="ignore-{{$item.id}}" href="javascript:doIgnoreThread({{$item.id}});" class="btn-link {{$item.ignore.classdo}}" title="{{$item.ignore.do}}"><i class="fa ri-notification-off-fill" aria-hidden="true"></i>&ensp;{{$item.ignore.do}}</a>
				</li>
				<li role="menuitem">
						<a id="unignore-{{$item.id}}" href="javascript:doIgnoreThread({{$item.id}});" class="btn-link {{$item.ignore.classundo}}"  title="{{$item.ignore.undo}}"><i class="fa ri-notification-3-fill" aria-hidden="true"></i>&ensp;{{$item.ignore.undo}}</a>
				</li>
				{{/if}}

				{{if $item.drop && $item.drop.dropping}}
				<li role="menuitem">
						<a class="btn-link navicon delete" href="javascript:dropItem('item/drop/{{$item.id}}', 'item-{{$item.guid}}');" title="{{$item.drop.label}}"><i class="fa ri-delete-bin-fill" aria-hidden="true"></i>&ensp;{{$item.drop.label}}</a>
				</li>
				{{/if}}

				{{if $item.block}}
				<li role="menuitem">
						<a class="btn-link navicon block" href="javascript:blockAuthor('item/block/{{$item.id}}', 'item-{{$item.guid}}');" title="{{$item.block.label}}"><i class="fa ri-prohibited-2-fill" aria-hidden="true"></i>&ensp;{{$item.block.label}}</a>
				</li>
				{{/if}}
				{{if $item.ignore_author}}
				<li role="menuitem">
						<a class="btn-link navicon ignore" href="javascript:ignoreAuthor('item/ignore/{{$item.id}}', 'item-{{$item.guid}}');" title="{{$item.ignore_author.label}}"><i class="fa ri-eye-off-fill" aria-hidden="true"></i>&ensp;{{$item.ignore_author.label}}</a>
				</li>
				{{/if}}
				{{if $item.collapse}}
				<li role="menuitem">
						<a class="btn-link navicon collapse" href="javascript:collapseAuthor('item/collapse/{{$item.id}}', 'item-{{$item.guid}}');" title="{{$item.collapse.label}}"><i class="fa ri-subtract-line" aria-hidden="true"></i>&ensp;{{$item.collapse.label}}</a>
				</li>
				{{/if}}
				{{if $item.ignore_server}}
				<li role="menuitem">
						<a class="btn-link navicon ignoreServer" href="javascript:ignoreServer('settings/server/{{$item.author_gsid}}/ignore', 'item-{{$item.guid}}');" title="{{$item.ignore_server.label}}"><i class="fa ri-eye-off-fill" aria-hidden="true"></i>&ensp;{{$item.ignore_server.label}}</a>
				</li>
				{{/if}}
				{{if $item.report}}
				<li role="menuitem">
						<a class="btn-link navicon ignore" href="{{$item.report.href}}"><i class="fa ri-alarm-warning-fill" aria-hidden="true"></i>&ensp;{{$item.report.label}}</a>
				</li>
				{{/if}}
			</ul>
		</span>
		...
		{{/if}}
			...
			<div class="wall-item-actions-items btn-toolbar btn-group visible-xs" role="group">
				...
				{{* Button to open the comment text field *}}
				{{if $item.comment_html}}
					<button type="button" class="btn button-comments" id="comment-{{$item.id}}" title="{{$item.switchcomment}}" {{if $item.thread_level != 1}}onclick="openClose('item-comments-{{$item.id}}'); commentExpand({{$item.id}});" {{else}} onclick="openClose('item-comments-{{$item.id}}'); commentExpand({{$item.id}});"{{/if}}><i class="fa ri-message-2-fill" aria-hidden="true"></i>
						<span title="{{$item.responses.comment.title}}">{{$item.responses.comment.total}}</span>
					</button>
				{{/if}}
						...
						<ul class="dropdown-menu dropdown-menu-left" role="menu" aria-labelledby="shareMenuOptions-{{$item.id}}">
							...
							<li role="menuitem">
								...
								<a class="btn-link" id="announce-{{$item.id}}" href="javascript:doActivityItemAction({{$item.id}}, 'announce', true);" title="{{$item.vote.unannounce.0}}">
									<i class="fa ri-prohibited-2-fill" aria-hidden="true"></i> {{$item.vote.unannounce.1}}
								</a>
								{{else}}
								<a class="btn-link" id="announce-{{$item.id}}" href="javascript:doActivityItemAction({{$item.id}}, 'announce');" title="{{$item.vote.announce.0}}">
									<i class="fa ri-repeat-line" aria-hidden="true"></i> {{$item.vote.announce.1}}
								</a>
								{{/if}}
							</li>
							{{/if}}
							...
							<li role="menuitem">
								<a class="btn-link" id="share-{{$item.id}}" href="javascript:jotShare({{$item.id}});" title="{{$item.vote.share.0}}">
									<i class="fa ri-chat-quote-line" aria-hidden="true"></i> {{$item.vote.share.1}}
								</a>
							</li>
							{{/if}}
							{{if $item.browsershare}}
							<li role="menuitem">
								<button type="button" class="btn-link button-browser-share" onclick="navigator.share({url: '{{$item.plink.orig}}'})" title="{{$item.browsershare.1}}">
									<i class="fa ri-stackshare-line" aria-hidden="true"></i> {{$item.browsershare.0}}
								</button>
							</li>
							{{/if}}
						</ul>
				...
				{{if $item.vote}}
					{{if $item.vote.like}}
					<button type="button" class="btn button-likes{{if $item.responses.like.self}} active" aria-pressed="true{{/if}}" id="like-{{$item.id}}" title="{{$item.vote.like.0}}" onclick="doActivityItemAction({{$item.id}}, 'like'{{if $item.responses.like.self}}, true{{/if}});" ><i class="fa ri-thumb-up-line" aria-hidden="true"></i>
						<span title="{{$item.responses.like.title}}">{{$item.responses.like.total}}</span>
					</button>
					{{/if}}
					{{if $item.vote.dislike}}
					<button type="button" class="btn button-likes{{if $item.responses.dislike.self}} active" aria-pressed="true{{/if}}" id="dislike-{{$item.id}}" title="{{$item.vote.dislike.0}}" onclick="doActivityItemAction({{$item.id}}, 'dislike'{{if $item.responses.dislike.self}}, true{{/if}});" ><i class="fa ri-thumb-down-line" aria-hidden="true"></i>
						<span title="{{$item.responses.dislike.title}}">{{$item.responses.dislike.total}}</span>
					</button>
					{{/if}}
				{{/if}}
				...
				{{* Event attendance buttons *}}
				{{if $item.isevent}}
				<button type="button" class="btn button-event{{if $item.responses.attendyes.self}} active" aria-pressed="true{{/if}}" id="attendyes-{{$item.id}}" title="{{$item.attend.0}}" onclick="doActivityItemAction({{$item.id}}, 'attendyes'{{if $item.responses.attendyes.self}}, true{{/if}});"><i class="fa ri-check-line" aria-hidden="true"><span class="sr-only">{{$item.attend.0}}</span></i>
					<span title="{{$item.responses.attendyes.title}}">{{$item.responses.attendyes.total}}</span>
				</button>
				...
				{{if $item.edpost || $item.tagger || $item.filer || $item.pin || $item.star || $item.follow_thread || $item.ignore || ($item.drop && $item.drop.dropping)}}
					<div class="more-links btn-group{{if $item.thread_level > 1}} dropup{{/if}}">
						<button type="button" class="btn dropdown-toggle" data-toggle="dropdown" id="dropdownMenuOptions-{{$item.id}}" aria-haspopup="true" aria-expanded="false" title="{{$item.menu}}"><i class="fa ri-more-2-line" aria-hidden="true"></i></button>
						<ul class="dropdown-menu dropdown-menu-right" role="menu" aria-labelledby="dropdownMenuOptions-{{$item.id}}">
							{{if $item.edpost}} {{* edit the posting *}}
							<li role="menuitem">
								<a href="javascript:editpost('{{$item.edpost.0}}?mode=none');" title="{{$item.edpost.1}}" class="btn-link navicon pencil"><i class="fa ri-edit-box-fill" aria-hidden="true"></i>&ensp;{{$item.edpost.1}}</a>
